...
void main() {
  testWidgets(
    'Buscar actualizaciones button enters checking state immediately '
        ...
        MaterialApp(
          home: Scaffold(
            body: SingleChildScrollView(
              child: UpdateSection(service: service),
            ),
          ),
        ),
      );
  ...
  testWidgets(
    'button resolves to upToDate banner when service returns '
    'available=false',
    (tester) async {
  ...
  testWidgets(
    'button resolves to error banner when the service throws',
    (tester) async {
      ...
      expect(find.text('No pudimos comprobar actualizaciones'),
          findsOneWidget);
      expect(find.textContaining('native channel not registered'),
          findsOneWidget);
    },
  );
}
...
class _StubUpdateService implements UpdateService {
  _StubUpdateService({required this.checkResult});
